...
import { Avatar,Snackbar, Divider, Button, Paper, Box, Grid, Typography, Container } from '@material-ui/core';
...
const Game = () => {
  ...
  const [playerMin, setPlayerMin] = useState(10)
  const [playerSec, setPlayerSec] = useState(60)
  const [opponentMin, setOpponentMin] = useState(10)
    ...
    if (game.game_over()) {
      if (!game.in_draw()) {
        ...
        if (game.turn() === playerColor.slice(0, 1)) {
          setOutcome(false)
          let outcome1 = 0
          socket.emit('gameOver', playerColor, roomcode, outcome1)
          // socket.on('elo', elo => {
          //   user.result.elo = elo
          ...
          setOutcome(true)
          let outcome2 = 1
          socket.emit('gameOver', playerColor, roomcode, outcome2)
          // socket.on('elo', elo => {
          //   user.result.elo = elo
          //   localStorage.setItem('profile', JSON.stringify(user))
          // })
        ...
        setGameOver(true)
        setOpen(true)
        setOutcome('The game resulted in draw.')
      }
    }
  ...
  useEffect(() => {
    socket.on('opponentConcede', (color) => {
      ...
      console.log(color1)
      socket.emit('gameOver', color1, roomcode, outcome1)
  ...
  const concede = () => {
    ...
    let concede = true
    console.log(playerColor)
    socket.emit('gameOver', playerColor, roomcode, outcome, concede)
    // socket.on('elo', elo => {
    //   user.result.elo = elo
    //   localStorage.setItem('profile', JSON.stringify(user))
    // })
            ...
            <Box style={p1header}>
              {loaded ?
                (
                  ...
                  : <Typography  component={'span'} style={opponentHeader}>
                    Please copy the link and sent it to your friend     <Button onClick={handleClick}> <FontAwesomeIcon icon={faCopy} size="xl" /> </Button>
                    <Snackbar
                      open={copyURL}
            ...
            <div style={chessboard}>
              ...
              {gameOver && (
                ...
                >
                  ...
                  <DialogContent>
                    <DialogContentText>
                      The game is over. {outcome ? 'You won the game.' : 'You lost the game.'}
                    </DialogContentText>
                  </DialogContent>
                  <DialogActions>
